		...
		var functions = 
			...
			validateString : function(type, string)
				...
				// Validate a simple URL regular expression
					...
					{
					var filter = /^[a-zA-Z]{3,4}(\d{6})((\D|\d){3})?$/i
					if(filter.test(string))
						{ return true; }
					else
						{ return false; }
			...
			validate : function(item)
				...
				if($(item).attr('validate')=='email')
					...
					var validType = 'web address';
                                ...
                                else if($(item).attr('validate')=='rfc')
					{
					var valid = functions.validateString('rfc', $(item).val());
					var validType = 'rfc';
					}
                                   else if($(item).attr('validate')=='aaaa')
					{
					var valid = functions.validateString('aaaa', $(item).val());
					var validType = 'aaaa';
					}
				if(!valid) 
					{
			...
			require : function(item)
				...
				if(!$(item).val() || $(item).val()==$(item).attr('place')) 
					...
					if(!$('#' + alertName).is(':visible'))
						{
						$('#' + formName).find('.formlyAlerts').append('<div class="formlyRequired formlyAlert" id="' + alertName + '">'+label+'Requerido</div>')
						$('#' + alertName).fadeIn();
						}
					var borderColor = $('#' + alertName).css('background-color');
					$(item).css('border-color', borderColor);
