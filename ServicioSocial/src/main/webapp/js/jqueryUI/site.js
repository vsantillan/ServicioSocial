...
var sh = {
	...
	config : {
		...
		strings : {
			...
			aboutDialog : '<center>SyntaxHighlighter<br>\
version 3.0.??? (February 20 2012)<br>\
http://alexgorbatchev.com/SyntaxHighlighter<br>\
JavaScript code syntax highlighter.<br>\
Copyright 2004-2010 Alex Gorbatchev.</center>'
		}
	},
...
$(document).ready( function () {
	$('div.collapse_details').css('display', 'none');

	$('div.collapse_details').each( function () {
		$(this).parents('dd').prev().append( '<a href="" class="show_hide">Show details</a>' );
		
		if ( $(this).children().length === 1 && $('dl.details', this).children().length === 0 ) {
			$(this).html( '<i>No additional information</i>' );
		}
	} );

	$('div.fw_docs a').each( function () {
		var href = $(this).attr('href');
		if ( href && href.indexOf( "DataTable.Editor" ) === 0 ) {
			this.href = "/docs/"+href;
		}
	} );
	...
	$(document).on('click', 'a.show_hide', function (e) {
		...
		var details = $(this).parents('dt').next().find('div.collapse_details');
	...
	$('.curr')
		.on( 'mouseenter', function () {
			...
			if ( ! this._curr ) {
				...
				this._curr = $(
					'<div class="curr_tooltip">'+
						'Â£'+price+' in other currencies using current exchange rates:'+
						'<table cellpadding="0" cellspacing="0" border="0">'+
							'<tr>'+
								'<td>$</td>'+
								'<td>'+((price * us_rate).toFixed(2))+'</td>'+
								'<td>(USD)</td>'+
							'</tr>'+
							'<tr>'+
								'<td>â‚¬</td>'+
								'<td>'+((price * euro_rate).toFixed(2))+'</td>'+
								'<td>(EUR)</td>'+
							'</tr>'+
							'<tr>'+
								'<td>Â¥</td>'+
								'<td>'+((price * yen_rate).toFixed(2))+'</td>'+
								'<td>(JPY)</td>'+
							'</tr>'+
						'</table>'+
					'</div>'
...
$(window).load( function () {
	...
	if ( window.location.hash && window.location.hash != "" )
	{
		...
		$("a[name="+sHash+"]").each( function () {
			$('a.show_hide', this.parentNode).click();
