 ...
 * @param <T>
 */
public class AbstractPlaticaDao<T extends Serializable>
{
...
//        getEntityMgr().clear();
        ...
        Query query = (Query) entityManager.createNativeQuery(sql, Platica.class);
        List<Platica> lista = query.getResultList();
        List<Platica> listaPlatica = new ArrayList<Platica>();

        for (Platica lista1 : lista)
        {
            listaPlatica.add(lista1);
        }
        return listaPlatica;
