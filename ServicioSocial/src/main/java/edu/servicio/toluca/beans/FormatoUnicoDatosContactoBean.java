...
//    @Pattern(regexp = numeros, message = errorNumeros+" en el campo Numero Interior")
    ...
    @Size(min = 0, max = 10, message = "El campo Telefono de Oficina debe ser a 10 digitos ")
...
//        if(referencias == null || referencias.equals("")){listaErrores.add("El campo Referencias no puede estar vacío"); }
        ...
        if (!mv.minimoString(calle, 1) && !mv.maximoString(calle, 200))
        {
            listaErrores.add("El campo calle debe tener entre 1 y 200 letras");
        }
        if (!mv.minimoString(numeroE, 1) && !mv.maximoString(numeroE, 5))
        {
            listaErrores.add("El campo numero exterior debe tener entre 1 y 5 digitos");
        }
        if (!mv.minimoString(entreCalles, 1) && !mv.maximoString(entreCalles, 255))
        {
            listaErrores.add("El campo entre calles debe tener entre 1 y 255 letras");
        }
        if (!mv.minimoString(referencias, 1) && !mv.maximoString(referencias, 70))
        {
            listaErrores.add("El campo referencias debe tener entre 1 y 70 letras");
        }

//        if(!mv.maximoString(numeroI, 5)){listaErrores.add("El campo numero interior no puede ser mayor a 5 digitos");} 
        if (!mv.maximoString(telefono_casa, 50))
        {
            listaErrores.add("El campo Telefono de casa no puede ser mayor a 50 digitos");
        }
        if (!mv.maximoString(telefono_cel, 30))
        {
            listaErrores.add("El campo Telefono celular no puede ser mayor a 30 digitos");
        }
        if (!mv.maximoString(telefono_oficina, 30))
        {
            listaErrores.add("El campo Telefono oficina no puede ser mayor a 30 digitos");
        }
        if (!mv.maximoString(correo_electronico, 30))
        {
            listaErrores.add("El campo Correo electronico no puede ser mayor a 30 digitos");
        }
